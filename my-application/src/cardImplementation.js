...
/**
 * List TemplateGroups and RuleTemplates in order to create Business Rule
 */
class BusinessRulesCreator extends React.Component {
    // Handles onChange of Select
    handleSelectChange = name => event => {
        console.log("Viewing RuleTemplates for : " + event.target.value)
        displayRuleTemplates(event.target.value)
    }

    constructor(props) {
        super(props);
        this.state = {
            // To store loaded ones
            templateGroups: getTemplateGroups(), // Get from API and store
            ruleTemplates: null,
            // To store selected ones
            selectedTemplateGroupName: null,
            selectedRuleTemplateName: null
        }
    }

    /////////////////////////////////////////////////////////////////////
    /**
     * Loads RuleTemplates, that belong to the selected TemplateGroup
     */
    ...
    render() {
        ...
        // To display each item in TemplateGroup selection todo: BusinessDomain refactor?
        var loadedTemplateGroups = this.state.templateGroups.map((loadedTemplateGroup) =>
            <MenuItem key={loadedTemplateGroup.name} value={loadedTemplateGroup.name}>
                {loadedTemplateGroup.name}
            </MenuItem>
        )

        // To display each item in RuleTemplate selection
        if(this.state.ruleTemplates !== null){
            var loadedRuleTemplates = this.state.ruleTemplates.map((loadedRuleTemplate) =>
            ...
            <div>
                ...
                <FormControl>
                    ...
                    <Select
                        ...
                        onChange={this.handleTemplateGroupSelected('age')}
...
class TemplateGroup extends React.Component {
    handleSelectChange = name => event => {
        console.log("Displaying for : " + this.state.name + " , " + event.target.value)
        displayCreateBusinessRuleForm(this.state.name, event.target.value)
    ...
    render() {
        // View Template Group as parent
        if (this.state.viewAs === "parent") {
            ...
            const ruleTemplates = this.state.ruleTemplates.map((ruleTemplate) =>
                <MenuItem key={ruleTemplate.name} value={ruleTemplate.name}>{ruleTemplate.name}</MenuItem>);
            return (
                <div>
                    <Typography type="headline" component="h2">
                        Rule Templates
                    ...
                    <div>
                        <FormControl>
                            <InputLabel htmlFor='ruleTemplate'>Select a Rule Template</InputLabel>
                            <Select
                                value='ruleTemplate'
                                onChange={this.handleSelectChange('ruleTemplate')}
                                input={<Input id='ruleTemplate'/>}
                            >
                                {ruleTemplates}
                            </Select>
                            <FormHelperText>Select a Rule Template to preview its properties below</FormHelperText>
                        </FormControl>
                    </div>
                </div>);
        }
...
class BusinessRule extends React.Component {
    ...
    //         <div>
    //             <FormControl>
    //                 <InputLabel htmlFor="businessRule">Select a Business Rule</InputLabel>
    //                 <Select
    //                     value={this.state.age}
    //                     onChange={this.handleChange('age')}
    //                     input={<Input id="age-helper" />}
    ...
    //                     <MenuItem value={30}>Thirty</MenuItem>
    //                 </Select>
    //                 <FormHelperText>The selected Business Rule can be edited or deleted</FormHelperText>
    //             </FormControl>
    //         </div>
    //     );
    // }
 ...
 * which consists of BusinessRulesCreator Creator & BusinessRulesCreator Modifier
 */
 ...
 */
function displayTemplateGroups() {
    ReactDOM.render(
...
function displayRuleTemplates(templateGroupName) {
    // Get Template Group with the given name
    var templateGroup = getTemplateGroup(templateGroupName)
    console.log("Found TemplateGroup :")
...
/**
 * Displays a form to fill in properties data, from a given RuleTemplate that belongs to the given TemplateGroup,
 * in order to create a Business Rule
 *
 * @param ruleTemplate Given Rule Template object, which the Business Rule uses
 */
function displayCreateBusinessRuleForm(templateGroupName, ruleTemplateName) {
    // Find the RuleTemplate with the given TemplateGroup & RuleTemplate
...
function displayEditBusinessRuleForm(templateGroupName, ruleTemplateName, businessRule) {
    ...
    ReactDOM.render(
        ...
        />, document.getElementById("form")); //todo: (Q) name 'form'?
}
